...
const PLAN_PROMPT = [
  ...
  "- If the request asks to revert, restore or go back to how something was,",
  "  the restore is ONE increment and it comes from git history (git log,",
  "  git show, git revert), not from a description of the old look. Any",
  "  'polish' or 'tidy' in the same request must keep the restored version",
  "  as it is and only fix what the request names -- it is not licence to",
  "  redesign or replace what was just restored. Say so in that increment's",
  "  request. (A 'revert the water and polish it' request once came back",
  "  with the old water restored in step one and rewritten into a third",
  "  look in step two.)",
  "",
